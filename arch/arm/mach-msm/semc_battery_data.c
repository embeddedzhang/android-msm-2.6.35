...
#define INTERPOLATE(IX, X1, Y1, X2, Y2) \
  ((Y1) + (((signed)((Y2) - (Y1))*(signed)((IX) - (X1)))/(signed)((X2) - (X1))))
...
static int compensate_batt_therm(struct data_info *di, int mv)
{
	...
	if (curr >= 0) {
		mv_cmp = mv * 1000 - curr * RESISTANCE_10_MOHM;
		mv_cmp /= 1000;
	} else {
		mv_cmp = mv;
	}
...
static s32 get_batt_therm_resistance(struct data_info *di, int *resistance)
{
	...
	mv = compensate_batt_therm(di, mv);

	if (mv >= BATTERY_THERM_VOLTAGE_MV)
		*resistance = UINT_MAX;
	else
		*resistance =
			(mv * BATTERY_THERM_DIVIDER_RESISTANCE_OHM) /
